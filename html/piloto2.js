...
let expInfo = {'participant': 'INGRESE SU CEDULA AQUÍ'};
...
function experimentInit() {
  ...
  text_subasta2 = new visual.TextStim({
    win: psychoJS.window,
    name: 'text_subasta2',
    text: 'Si la valoración que usted dio es MAYOR a la respuesta que genere la computadora, se le entregará el producto por el cuál realizó esa oferta y se le entregará el resultado de la resta de los 15.000 pesos y el valor del producto.\n\nSi la valoración que usted dio es MENOR a la respuesta que genere la computadora, se le entregarán los 15.000 pesos que tenía inicialmente.',
    font: 'Arial',
    units: undefined, 
    pos: [0, 0], height: 0.05,  wrapWidth: undefined, ori: 0,
    color: new util.Color('black'),  opacity: 1,
...
function text_pagoRoutineBegin(snapshot) {
  return function () {
    ...
    for (const thisComponent of text_pagoComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    // check if the Routine should terminate
    if (!continueRoutine) {  // a component has requested a forced-end of Routine
...
function pago3RoutineBegin(snapshot) {
  return function () {
    ...
    if (precio_sujeto > precio_aleatorio) {
        ...
        mensaje1 = `Usted ha ganado 15.000 pesos`;
    }
